...
The Model settings page uses the same menu selection pattern as the panel. It shows the active software and effective model together, then offers only models for that connection. API profiles are keyed by normalized scheme, host, port and path; CLI profiles include the CLI provider and executable path. Switching connections restores the matching Quick/Deep choices. A trailing slash or capitalization change in an endpoint does not create a duplicate profile.
