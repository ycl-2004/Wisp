...
2026-09-12 单光标修正：已移除会形成残留的共享静止箭头。当前只创建一个私有移动箭头，兼容录屏中可能不显示鼠标。以下静止箭头测试属于历史实现，已由[单光标修复](single-cursor-20260912.md)取代。

2026-09-12 更新：[光标锁定与点击反馈修复](cursor-lock-repair-20260912.md)。当前开关统一箭头样式与按钮反馈，并覆盖更多本进程表面；录制端的点击圆圈仍需在录制工具中关闭。下文 2026-09-11 的菜单退出和独立箭头样式描述属于历史版本。

2026-09-11 补充：[本地光标实验](#本地光标实验)现已改为本地移动箭头加共享静止箭头，并完成非音频回归；详见[全链路验收报告](non-audio-acceptance-20260911.md)。下文其他章节保留各自日期的历史证据，不能代替当前构建复测。2026-09-10 补充：[原生能力研究与扩展捕获矩阵](screen-privacy-research-20260910.md)。
...
当前修复测试日期：2026-09-11，macOS 26.6.2（25G83）。开关现名「隐私光标锁定（实验）」，默认关闭，需同时开启窗口隐藏。私有非激活窗口绘制跟随鼠标的本地箭头；另一个仅含箭头的共享窗口停在进入位置，置于 Wisp 下方。系统指针暂时隐藏，事件不被拦截、重放或改写，也不移动系统鼠标。非激活但持有键盘焦点的面板也能使用。离开、原生菜单、失活、关闭选项和正常退出会恢复光标。

独立捕获进程使用 `SCStreamConfiguration.showsCursor = true`，`excludingWindows: []`，没有替测试应用配置排除列表。最新一次预热后的采样为关闭 16 帧、开启 15 帧、恢复 14 帧：关闭及恢复各观察到多个箭头位置；开启组所有帧中箭头边界一致，本地箭头位置同时改变。青色背景对照始终存在，Wisp 洋红标记始终为 0。系统区域视频每组从多个时间点选取一个有完整控制像素的采样，但不能据单帧证明完整视频静止。

**静态 ScreenCaptureKit 和系统区域截图在开启时没有箭头像素。** 视频结果不能推广到静态截图。单独传输鼠标元数据的工具也可能显示真实位置；若捕获器不遵守窗口隐藏，还可能看到本地箭头和 Wisp。本方案不提供通用系统级捕获隔离。

原生合成窗口实测：点击 1 次、输入 `cursor test`、选区长度 11、滚动偏移 140；拖动、缩放均改变 frame，离开后两个箭头窗口收起并恢复系统光标。最新结果位于 `/private/tmp/wisp-nonaudio.DJthrv/cursor/result.json`，摘要见[当前验收记录](non-audio-acceptance-20260911.md)。早期「开启后所有鼠标像素为 0」是旧版本行为，不再作为新功能的成功标准。

本轮真实 Chromium 的页面正文和监测 UI 测试使用合成页面/合成视频流；没有把它计作真实桌面 `getDisplayMedia` 或第三方接收端验证。Zoom、Meet、Teams、飞书、OBS、Screen Studio、远程桌面仍需逐一复核。
