...
> **当前分发状态：** 最新版本是 `v0.3.0 (build 5)`，Universal 2 构建，含 `arm64` 与
...
- **本地光标（实验）。** 在「设置 → 权限 → 屏幕共享」中可以让 Wisp 在自己的窗口内绘制
  光标，同时隐藏系统光标。显示的光标始终是箭头，包括靠近缩放边缘时；点击、选字、滚动、
  拖动和缩放仍使用原生事件。默认关闭，且需要
  同时开启 Wisp 窗口隐藏；本地 ScreenCaptureKit、系统截图和视频对照中，开启时没有看到
  光标像素，关闭后恢复。这不代表所有浏览器、会议软件、录屏工具或远程桌面都遵守同样结果。
- **缩放提示。** 面板内侧新增一圈圆角淡蓝色细线，以及四条边中间的小标记，提示自定义
  缩放的可拖拽位置。它只负责提示，不会拦截点击，光标仍保持箭头。
...
当前版本是 `0.3.0 (build 5)`，见 [CHANGELOG.md](CHANGELOG.md)，对应 Git 标签
`v0.3.0`。
