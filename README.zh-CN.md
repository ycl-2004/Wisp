...
- 默认开启：Wisp 的窗口被标记成其他进程读不到（`NSWindow.sharingType = .none`），
  Zoom、Google Meet、腾讯会议、飞书、Teams、QuickTime、系统录屏（`⌘⇧5`）和 OBS
  抓到的画面里都没有它，你自己的屏幕上照常显示。这条限制由 WindowServer 执行，
  所以一处设置挡住所有录屏工具，不需要逐个 App 适配；共享时的窗口选择列表里也不会出现 Wisp。
- 菜单栏里可以直接开关，也可以走「设置 → 权限 → 屏幕共享」。要录 Wisp 自己的演示视频时关掉它。
- 菜单栏那颗图标盖不住：第三方菜单栏图标由系统代为绘制，应用自己持有的 `NSStatusBarWindow`
  只是个 35×0 的空壳，改它的 sharingType 没有效果，所以图标仍会出现在录像里。
  介意的话用菜单栏管理工具把它收起来，快捷键和小药丸照样能唤起 Wisp。
- 另外挡不住的：用手机对着屏幕拍，以及在显示器输出端接硬件采集卡。
