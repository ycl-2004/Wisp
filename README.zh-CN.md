...
接着它会截取当前窗口（也可以设成截整个屏幕）；如果最前面的是支持的浏览器，还会读出网址、标题、选中的文字和整页正文。
不用再在两个应用之间来回复制粘贴。

它是一层本地优先的桌面外壳，模型由你自己选：OpenAI 兼容接口、Ollama，或者 Agent CLI（包含 Codex、Antigravity 与 Claude Code）。
对话文字和页面文字快照存在你自己的 Mac 上；本地 CLI 可能把当次截图写进一个私有临时目录，命令结束后由 Wisp 删除。
界面提供简体中文和英文，跟随系统语言。

> **当前分发状态：** 最新版本是 `v0.4.0 (build 6)`，Universal 2 构建，含 `arm64` 与
> `x86_64`。发布包是 ad-hoc 签名、未经 Apple 公证，
> 首次打开可能需要按住 Control 点击 → **打开**。

> **会被发出去的东西：** 用云端接口时，你正在看的那一页的**整页正文**和当前窗口（或整个屏幕）的**截图**
> 会发到你配置的地址。排除机制是按应用而不是按站点的 —— 详见 [PRIVACY.md](PRIVACY.md)。


输入框上方可选择「快速／深入」，默认是快速。在「设置 → 模型」中分别指定两种模式的模型和快捷键；回答过程中仍可修改，下次提问生效。快速模式跳过新的整页正文采集，直接用默认模型；深入模式在支持时提高推理力度，在 Antigravity 上自动换成默认模型的 High 思考档。两种模式都优先请求可用的 Fast。快速模式的回答旁边有「深入重答」，一键用深入模式替换这条回答，不多占一轮。秒表显示准备、首字及总耗时。深入不代表答案已经验证；详见[模式、加速与准确性验证](docs/model-speed.md)。

「设置 → 指令」里存着常用指令（例如「总结这一页」「翻译选中内容」），每条可以指定自己的回答模式和快捷键，在输入框旁的 ✦ 菜单、空白对话里或任何应用里用快捷键都能触发。输入框里写了的文字会作为材料跟在指令后面。
...
「设置 → 音频 → 识别引擎」除了 Apple Speech，还会列出 `~/Documents/huggingface/models/`
里找到的本地 sherpa-onnx 模型（目前识别 SenseVoice 和 Qwen3-ASR 两种架构）。再放进一个
模型文件夹，点「重新扫描」就能选，不需要重装 Wisp。本地模型的中文默认统一成简体（Qwen3-ASR 有时会写繁体），
可在「设置 → 音频 → 中文字形」里改。详情见[识别引擎说明](docs/live-listening.md#recognition-engines)。

「按住说话提问」（设置 → 音频 → 快捷键，默认未设置）：按住快捷键说出问题，松开后连同当前屏幕上下文，
用当前回答模式直接发送。只用麦克风，不写进录音记录；录音进行中不可用。按一下不到 0.3 秒不会发送；
超过 60 秒则只放进输入框、不自动发送。
...
5. 打开**设置 → 模型**，选一种接法（改动自动保存），点**测试连接**。
...
- **模型连接是你自己的。** 云端兼容接口、本地 Ollama 模型，或者你已经登录的 Agent CLI（Codex、Antigravity 或 Claude Code）。
...
- 也可以改成截焦点窗口所在的整块屏幕，一次问好几个窗口。排除的应用和你勾选的应用会从画面里挖掉。
...
- 虚拟滚动页面采集结束后会清理临时的 `window.__wispCollector`；Wisp 只读取
  Chromium 偏好来判断 Apple Events 开关，不会把 Wisp 数据写进浏览器配置、历史或缓存。
...
- 新增[原生能力研究与扩展测试](docs/screen-privacy-research-20260910.md)，覆盖独立捕获进程、
  录制中切换过滤器、系统视频录制，以及受保护视频层和专用共享窗口等替代方案。
  说明气泡、上下文与耗时详情、许可说明页在挂载到窗口时应用隐藏偏好，减少时机缺口；
  这不改变下面的系统能力边界。
- 默认开启，通过 `NSWindow.sharingType = .none` 请求隐藏窗口，本机仍可使用 Wisp。
  Apple 已将它列为旧机制，并明确要求不要依赖它阻止捕获；具体效果取决于 macOS
  和录屏方式，开关开启不代表接收端已经看不到 Wisp。
- 菜单栏里的「尝试在屏幕共享时隐藏」可以直接开关，也可以走「设置 → 权限 → 屏幕共享」。
  要录制 Wisp 演示时请关闭。
- 在 macOS 26.6.1 的本地测试中，使用 Wisp 生产隐藏代码的合成窗口没有出现在
  ScreenCaptureKit 整屏截图和视频帧中，但仍出现在可捕获窗口列表里。
  这不代表 Chrome、Zoom、Meet、飞书、Teams、OBS 或监考平台都经过验证。
  详见[测试范围、结果与复现步骤](docs/screen-privacy-validation.md)。
- 菜单栏图标、系统弹窗不保证隐藏。这一设置不隐藏应用身份、焦点变化、剪贴板事件
  或第三方的行为记录，也不影响摄像机拍屏和硬件采集。使用前请检查实际接收端画面。
- **隐私光标锁定（实验）。** 在「设置 → 权限 → 屏幕共享」中开启后，先隐藏系统指针，再显示
  一个跟随鼠标的本地私有箭头。已移除共享静止箭头，避免它从透明或移动的窗口下露出，形成
  双鼠标。同时统一文本、控件和缩放边缘的箭头样式，并关闭自定义按钮及普通按钮
  的按压特效。关闭后恢复原生光标请求和按钮反馈。受保护的面板、设置标题栏、非焦点浮窗以及
  本进程菜单、气泡使用相同策略，原生拖动持续到松手。后台悬停保留系统指针，点击时先激活
  Wisp 再锁定，并保存原外部采集目标；本机保留文字选区、输入光标与菜单选中提示，
  点击、输入、选区、滚动保持原生行为。这些提示随所属窗口一起向兼容录屏请求隐藏。
  默认关闭；开启时同时开启窗口隐藏，关闭窗口隐藏也关闭锁定，单独关闭锁定则保留窗口隐藏。
  **录屏工具仍可能独立绘制点击圆圈或传输鼠标坐标。** 点击提示需在录制工具中关闭，Wisp 无法
  修改其他 App 的录制流。兼容录屏在操作 Wisp 时省略鼠标，不再人为补画静止箭头。
  详见[单光标修复验证](docs/single-cursor-20260912.md)。
- **缩放提示。** 面板内侧新增一圈圆角淡蓝色细线，以及四条边中间的小标记，提示自定义
  缩放的可拖拽位置。它只负责提示，不会拦截点击；开启隐私光标锁定时，缩放光标保持箭头。
...
- **Agent CLI：** 同一个设置分组里可以选 Codex、Antigravity 或 Claude Code。Codex 运行本机的 `codex app-server --listen stdio://`，创建临时、只读的会话；Antigravity 和 Claude Code 使用各自的 headless `stream-json`。三者都随文字增量刷新答案，截图通过私有临时工作目录传入，都不往 Wisp 的对话目录里写会话文件。协议与验证见 [CLI 流式接入说明](docs/cli-streaming.md)。
  支持的 Codex 模型和 Claude Opus 选项默认优先 Fast，不可用时回到普通速度；AGY 1.1.27 没有 Fast 开关。Fast 可能增加额度消耗，不降低思考强度，也不替换所选模型。
...
- **Agent CLI：** 在同一个分组里选择 Codex、Antigravity 或 Claude Code，再选检测到的可执行文件和模型。用的是你 Mac 上已有的 CLI 登录。Antigravity 模型列表每次通过 `agy models` 重新扫描，Antigravity 自己更新后不需要改 Wisp。

三种接法都有**测试连接**。云端和 Ollama 的测试会给快速、深入实际使用的模型各发一张很小的测试图；Codex 与 Antigravity
检查 `--version`，Claude Code 检查 `auth status`，因此未登录会得到单独提示。本地 CLI 的检查都不消耗模型请求。
...
- Codex 接法由 Wisp 启动独立的本地 app-server 进程，为图片输入建一个临时目录，以 `ephemeral: true`、只读沙箱和 `approvalPolicy: never` 创建会话；回答结束后关闭进程并删除目录。Codex 自己的账号、网络和服务端日志不在 Wisp 的控制范围内。
- Antigravity 接法由 Wisp 启动本地 `agy` 进程，带 `--sandbox`，跑在自己的临时工作目录里，走 Antigravity 的 JSON headless 模式。有截图时，截图写进这个目录并在提问里点名路径，由 Antigravity 自己读取；请求结束就把目录删掉。Antigravity 自己的账号、网络、额度和服务端日志不在 Wisp 的控制范围内。
...
同时，单独豁免某一个网址或域名。整屏截图会挖掉排除的应用，以及你在「设置 → 采集 → 截图范围」里勾选的应用。
...
当前版本是 `0.4.0 (build 6)`，见 [CHANGELOG.md](CHANGELOG.md)，对应 Git 标签
`v0.4.0`。
...
```bash
tools/install-local.sh              # 构建、按团队证书签名、安装；验证后清理临时备份
tools/install-local.sh --no-install # 只构建和签名
tools/install-local.sh --keep-backup # 可选：保留回滚备份
```
...
安装脚本只在替换和签名校验期间暂存旧包，默认校验成功后立即清理；需要回滚时再加
`--keep-backup`。
...
- Codex 流式接入需要支持 app-server v2 协议的 CLI，已用 0.153.4 验证；每次请求仍带着 Codex 自己的固定上下文开销。
- Antigravity 按 agent response 增量显示答案，已用 1.1.27 验证；只发送最终结果的旧版本会在结束时显示全文。每次请求仍带上它自己的固定上下文开销——此前测量在 Wisp 的内容之前约 30,400 token。Antigravity 的 headless 输入只接受文字，会拒绝 `image_url` 内容块，所以截图是以文件形式传入的，带截图的请求要多花一轮读取文件的工具调用，代价约 1,150 token，而不是把图内联进上下文。Antigravity 还会在约 71,400 token 处静默截断自己的输入，因此 Wisp 会先把提示压进一个更小的预算并标注省略了什么；远超默认 60,000 字上限的页面因此是「删节后送达且写明缺口」，而不是悄悄少一截。
