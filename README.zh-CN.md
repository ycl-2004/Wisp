...
输入框上方可选择「快速／深入」，默认是快速。在「设置 → 模型」中分别指定两种模式的模型和快捷键；回答过程中仍可修改，下次提问生效。快速模式跳过新的整页正文采集，深入模式在支持时提高推理力度，两种模式都优先请求可用的 Fast。秒表显示准备、首字及总耗时。深入不代表答案已经验证；详见[模式、加速与准确性验证](docs/model-speed.md)。
...
三种接法都有**保存并测试连接**。云端和 Ollama 的测试会发一张很小的测试图；Codex 与 Antigravity
