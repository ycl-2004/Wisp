...
enum PromptBuilder {
    ...
    static func contextBlock(_ context: ContextSnapshot, full: Bool) -> String {
        guard full else {
            var line = "[较早一轮的屏幕上下文，已折叠] \(context.summaryLine)"
            if let total = context.pageTextTotalChars, total > 0 {
                line += "（当时读到正文 \(total) 字，此处不再重复）"
            }
            return line
        }

        var lines: [String] = ["<屏幕上下文>"]
        lines.append("应用：\(context.appName)")
        if let title = context.windowTitle, !title.isEmpty { lines.append("窗口标题：\(title)") }
        if let url = context.url, !url.isEmpty { lines.append("网址：\(url)") }
        if let pageTitle = context.pageTitle, !pageTitle.isEmpty { lines.append("页面标题：\(pageTitle)") }
        lines.append("截取时间：\(Self.timeFormatter.string(from: context.capturedAt))")
        lines.append("截图：\(context.hadScreenshot ? "有，只覆盖当前可视区域" : "无")")

        if !context.iframeURLs.isEmpty {
            lines.append("读不到内容的嵌入框架（\(context.iframeURLs.count) 个）：")
            for url in context.iframeURLs.prefix(8) { lines.append("  - \(url)") }
        }

        if let selection = context.selectedText, !selection.isEmpty {
            lines.append("用户选中的文字：")
            lines.append(selection)
        }

        if let text = context.pageText, !text.isEmpty {
            let total = context.pageTextTotalChars ?? text.count
            let truncatedNote = total > text.count ? "，原文共 \(total) 字，中间已省略" : ""
            lines.append("整页正文（\(text.count) 字\(truncatedNote)）：")
            lines.append("---")
            lines.append(text)
            lines.append("---")
        } else {
            lines.append("整页正文：未取到，只能依赖截图。")
        }
